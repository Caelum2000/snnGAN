...
def update_D(X, Z, net_D, net_G, trainer_D, img_size,channels):
    ...
    if glv.network_config['net_D_direct_input']:
        fake_X = fake_X.reshape((n_step, batch_size, channels, img_size, img_size))
    else:
...
if __name__ == "__main__":
    # print("This is Moba")
    data_path = "./data"
    config = "./NetworkConfigs/wgan_cifar10.yaml"
    params = Parse(config)
    ...
    if glv.network_config['dataset'] == 'MNIST':
        ...
        trainloader, _ = load_datasets.load_cifar10(
            data_path, is_normlized=glv.network_config['is_data_normlized'])
    ...
    if glv.network_config['net_G'] == 'Generator':
        ...
        net_G = DGAN_snn.Generator_3_MP_Scoring_2(
            input_dim=glv.network_config['latent_dim'])
    elif glv.network_config['net_G'] == 'Generator_2_MP_CIFAR10':
        net_G = DGAN_snn.Generator_2_MP_CIFAR10(
            input_dim=glv.network_config['latent_dim'])
    else:
    ...
    elif glv.network_config['net_D'] == "Discriminator_3_MP_CIFAR10":
        net_D = WGAN_snn.Discriminator_3_MP_CIFAR10()
    else:
        ...
        with torch.no_grad():
            ...
            if glv.network_config['compute_fid']:
                fid_score = clean_fid.get_clean_fid_score(
                    net_G, glv.network_config['dataset'], device, num_gen=5000)
